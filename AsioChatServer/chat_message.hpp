...
public:
	...
	void package()
	{
		...
		std::sprintf(header, "%4d%4d", static_cast<int>(name_length_), static_cast<int>(body_length_));
	...
	bool unpackage()
	{
		...
		std::strncat(body_length_buffer, data_ + header_name_length, header_body_length);
